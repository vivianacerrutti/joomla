    ...
    function view() {
        ...
        if (trim($this->formrow->template_code_processed) == '') {
            ...
            if( $is_mobile_type == 'choose' ) {
                ...
                echo '<div style="display: block; text-align: center;"><button class="ff_elem" onclick="location.href=\''.$return_url.'\';"><span>'.JText::_('COM_BREEZINGFORMS_MOBILE_VERSION').'</span></button></div><div></div>';
            }
                    ...
                    foreach ($this->maildata as $DATA) {
                        if( strtolower($DATA[_FF_DATA_NAME]) == strtolower($field) ){
                            if(isset($froms[1])){
                                ...
                                $recipients[$i] = $DATA[_FF_DATA_VALUE];
                            }
                            break;
                        }
                    }
                                    ...
                                    try{
                                        ...
                                        if( $this->formrow->dropbox_email && $this->formrow->dropbox_password ){
                                           ...
                                           foreach($dropbox_types As $dropbox_type){
                                               $dropbox_file = '';
                                               switch($dropbox_type){
                                                   case 'pdf': $dropbox_file = $this->exppdf(); break;
                                                   case 'csv': $dropbox_file = $this->expcsv(); break;
                                                   case 'xml': $dropbox_file = $this->expxml(); break;
                                               }
                                               if($dropbox_file != ''){
